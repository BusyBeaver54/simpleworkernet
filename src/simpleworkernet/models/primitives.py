...
class GeoPoint(BaseModel):
    ...
    legacy=True: совместимость со старым AUTOCAD (фиксированный TO_KM=0.6194
    на абсолютных mercator-координатах вместо relative × cos(lat)).
    """
    ...
    def to_xy(
        ...
        *,
        ...
        # legacy=True: AUTOCAD-compatible fixed TO_KM scale on absolute mercator
        if projection == "mercator" and legacy:
            x, y = _mercator_raw(float(self.lat), float(self.lon))
            x, y = x * LEGACY_TO_KM, y * LEGACY_TO_KM
            if c is not None and not absolute:
                cx, cy = _mercator_raw(float(c.lat), float(c.lon))
                cx, cy = cx * LEGACY_TO_KM, cy * LEGACY_TO_KM
                x, y = x - cx, y - cy
            x = x * scale + offset[0]
            y = y * scale + offset[1]
            if rotation_deg:
    ...
    def from_xy(
        ...
        *,
        ...
        if projection == "mercator":
            ...
            if legacy:
                dx, dy = dx / LEGACY_TO_KM, dy / LEGACY_TO_KM
            elif auto_scale_mercator:
                k = _mercator_metric_scale(float(c.lat))
                if abs(k) < 1e-15:
