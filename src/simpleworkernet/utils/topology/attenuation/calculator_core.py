...
class AttenuationSegmentsMixin:
    ...
    def _fiber_length(self, fiber_id, fiber_obj) -> Tuple[Optional[float], str]:
        slack = 1.03
        try:
            # per-cable geo_slack_k если catalog поддерживает
...
class AttenuationEdgeMixin:
    ...
    def _point_from_vattrs(self, vattrs: dict) -> EndpointInfo:
        """Краткая EndpointInfo по атрибутам вершины (для start/end сегмента)."""
        if not vattrs:
            return EndpointInfo()
        ...
        try:
            ...
            port_i = None
        name = _obj_display_name(vattrs)
        node_id = vattrs.get("node_id")
        try:
            node_id = int(node_id) if node_id is not None else None
        except (TypeError, ValueError):
            node_id = None
        port_name = vattrs.get("port_name")
        if ot == TYPE_FIBER and hasattr(self, "_fiber_core_info"):
            try:
                core = self._fiber_core_info(vattrs)
                if core.get("port") is not None:
                    port_i = int(core["port"])
                if core.get("port_name"):
                    port_name = core["port_name"]
        ...
        return EndpointInfo(
            ...
            port=port_i,
            port_name=str(port_name) if port_name else None,
            label=_label_vertex(vattrs) if vattrs else "",
        )

    def _stamp_segment_ends(
        self,
        segs: List[AttenuationSegment],
        ua: dict,
        va: dict,
    ) -> List[AttenuationSegment]:
        """Проставить start/end на сегменты стыка ua→va."""
        if not segs:
            return segs
        start = self._point_from_vattrs(ua)
        end = self._point_from_vattrs(va)
        for s in segs:
            if s.start is None:
                s.start = start
            if s.end is None:
    ...
    def _edge_segments(
        ...
        if (
            ...
            return self._stamp_segment_ends(segs, ua, va)
    ...
    def _connector_db_triple(self):
        wl = getattr(self, "wavelength", None)
        if hasattr(self.catalog, "connector_db_triple"):
            try:
                return self.catalog.connector_db_triple(wavelength_nm=wl)
            except TypeError:
                return self.catalog.connector_db_triple()
        try:
            db = self.catalog.connector_db(wavelength_nm=wl, use_max=self.use_max)
        except TypeError:
    ...
    def _adapter_db_triple(self):
        ...
        if hasattr(self.catalog, "adapter_db_triple"):
            try:
                return self.catalog.adapter_db_triple(wavelength_nm=wl)
            except TypeError:
                return self.catalog.adapter_db_triple()
        try:
            db = self.catalog.adapter_db(wavelength_nm=wl, use_max=self.use_max)
        except TypeError:
            db = self.catalog.adapter_db(use_max=self.use_max)
        return db, db, db

    def _splice_db_triple(self):
        wl = getattr(self, "wavelength", None)
        if hasattr(self.catalog, "splice_db_triple"):
            try:
                return self.catalog.splice_db_triple(wavelength_nm=wl)
            except TypeError:
                return self.catalog.splice_db_triple()
        try:
            db = self.catalog.splice_db(wavelength_nm=wl, use_max=self.use_max)
        except TypeError:
    ...
    def _external_joint_segments(
        ...
        if ta == TYPE_SPLITTER and tb == TYPE_SPLITTER and ida != idb:
            db_min, db, db_max = self._splice_db_triple()
        ...
        if TYPE_CROSS in kinds:
            ...
            # Пара кабель–адаптер (fiber↔cross): дополнительно splice (сварка).
            ...
            # кабель–адаптер: сварка между fiber и cross
            if TYPE_FIBER in kinds:
                db_min, db, db_max = self._splice_db_triple()
                out.append(AttenuationSegment(
                    kind="splice", db=db, db_min=db_min, db_max=db_max,
                    description=(
                        f"splice from {self._iface_ref(ua)} to {self._iface_ref(va)}"
                    ),
                    source="default", wavelength_nm=self.wavelength, meta=meta,
                ))

            if cross_is_endpoint:
                out.append(self._cross_connector_segment(
