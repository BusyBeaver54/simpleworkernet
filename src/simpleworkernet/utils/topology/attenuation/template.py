...
from .catalog import guess_ratio_key, ports_from_ratio_key

def _shared_config_dir() -> Path:
    """Каталог attenuation.json — config_manager.attenuation_json_dir или XDG."""
    try:
        from ....core.config import config_manager
        d = config_manager.attenuation_json_dir
        if d:
            p = Path(d)
...
def attenuation_json_path(
    ...
    *,
    ...
    try:
        from ....core.config import config_manager
        return config_manager.attenuation_json_path(key)
    except Exception:
